import { listAdditionalServiceFees } from "@/modules/additional-service-fees";
...
/** Phí vỏ thùng từ package-configuration Customer chọn — khóa trên màn Sales. */
export function isPackingFee(fee) {
  const code = String(fee?.code ?? "").toUpperCase();
  const feeType = String(fee?.feeType ?? fee?.ruleType ?? "").toUpperCase();
  return (
    code === "PACKING_FEE" ||
    feeType === "PACKING_FEE" ||
    code.includes("PACKING_FEE") ||
    feeType.includes("PACKING")
  );
}
...
export function applySelectedPricingRuleIdsToFeeLines(
  ...
  return lines.map((line) => {
    ...
    const alwaysOn =
      ...
      isDomesticFee(line) ||
      isPackingFee(line) ||
      (catalogFee ? isDomesticFee(catalogFee) || isPackingFee(catalogFee) : false);
...
export function buildConsignmentQuotationDraft({
  ...
  return {
    ...
    chargeableWeight,
    mainServiceAmount,
    additionalFees: enabledFeeLines,
...
function resolveLineAmountFromApiFee(fee, { enabled, context, catalogFee }) {
  ...
  // PACKING_FEE: BE có thể gộp nhiều kiện vào amount trong khi quantity=1 — ưu tiên amount API.
  if (isPackingFee(fee) && fee.amount != null && fee.amount !== "") {
...
export function buildAdditionalFeeLinesFromQuotation(
  ...
  if (apiFees.length) {
    return apiFees.map((fee) => {
      ...
      const isRequired =
        fee.isRequired === true || catalogFee?.isRequired === true || packing;
  ...
  try {
    ...
    return {
      ...
      quotation: {
        ...
        domesticShippingFee:
          local.quotation?.domesticShippingFee ?? existing.quotation?.domesticShippingFee,
        additionalFees:
...
/**
 * Lựa chọn thùng Customer (từ items đơn ký gửi) + phí PACKING_FEE trên báo giá.
 * Không hard-code phí/kích thước — lấy từ nested packageConfiguration / additionalFees.
 */
export function resolveCustomerPackageSelections(consignment, quotation) {
  const items = Array.isArray(consignment?.items) ? consignment.items : [];
  const quote = quotation ?? consignment?.quotation ?? null;
  const packingFees = (quote?.additionalFees ?? []).filter((fee) => isPackingFee(fee));
  const packingFeeTotal = packingFees.reduce(
  ...
  const selections = items
    .map((item, index) => {
      ...
      const packageConfigurationId =
        item.packageConfigurationId ?? config?.id ?? null;
      if (!packageConfigurationId && !config) return null;

      const actualConfig = item.actualPackageConfiguration ?? null;
      const actualPackageConfigurationId =
        item.actualPackageConfigurationId ?? actualConfig?.id ?? null;

      return {
        itemId: item.id ?? null,
        itemIndex: index,
        productName: item.productName ?? `Mặt hàng ${index + 1}`,
        ...
        typeMismatch:
          actualPackageConfigurationId &&
          packageConfigurationId &&
          String(actualPackageConfigurationId) !== String(packageConfigurationId),
      };
    })
    .filter(Boolean);
  ...
  return {
    ...
    packingFees,
    packingFeeTotal,
    hasCustomerSelection: selections.length > 0,
  };
}

function dimsKey(source) {
...
/**
 * Cảnh báo lệch thùng thực tế vs Customer chọn.
 * Ưu tiên so `packageConfigurationId` / configCode; fallback heuristic kích thước parcel.
 */
export function buildPackageConfigMismatchWarnings({ selections = [], parcels = [] } = {}) {
  const warnings = [];

  for (const selection of selections) {
    if (selection.typeMismatch) {
      const expected =
        selection.config?.code || selection.config?.configCode || selection.packageConfigurationId;
      const actual =
        selection.actualConfig?.code ||
        selection.actualConfig?.configCode ||
        selection.actualPackageConfigurationId;
      warnings.push({
        level: "type",
        itemId: selection.itemId,
        productName: selection.productName,
        message: `Loại thùng thực tế (${actual}) khác lựa chọn Customer (${expected}) cho “${selection.productName}”.`,
      });
    }
  }

  if (!parcels?.length || !selections.length) return warnings;

  for (let i = 0; i < parcels.length; i += 1) {
    const parcel = parcels[i];
    const selection = selections[i] ?? selections[0];
    if (!selection?.config) continue;

    if (parcel.packageConfigurationId && selection.packageConfigurationId) {
      if (String(parcel.packageConfigurationId) !== String(selection.packageConfigurationId)) {
        warnings.push({
          level: "type",
          itemId: selection.itemId,
          parcelId: parcel.parcelId,
          productName: selection.productName,
          message: `Kiện ${parcel.packageCode || parcel.parcelId || i + 1}: loại thùng kho khác lựa chọn Customer (${selection.config.code || selection.packageConfigurationId}).`,
        });
        continue;
      }
    }

    const expectedDims = dimsKey(selection.config);
    const actualDims = dimsKey(parcel);
    if (expectedDims && actualDims && expectedDims !== actualDims) {
      warnings.push({
        ...
        parcelId: parcel.parcelId,
        productName: selection.productName,
        message: `Kiện ${parcel.packageCode || parcel.parcelId || i + 1}: kích thước kho (${actualDims.replace(/x/g, " × ")} cm) khác chuẩn thùng Customer (${expectedDims.replace(/x/g, " × ")} cm).`,
      });
    }
  }
...
export function getQuotationDisplayLines(quotation) {
  ...
  const domesticShippingFee = Number(quotation.domesticShippingFee) || 0;
  ...
  if (
    ...
    // BE có thể trả domesticShippingFee riêng (không nằm trong additionalFees).
    const hasDomesticLine = enabledFees.some((fee) => isDomesticFee(fee));
    if (!hasDomesticLine && domesticShippingFee > 0) {
      lines.push({
...
export function mergeSentQuotationSnapshot(apiQuotation, localQuotation) {
  ...
  return {
    ...
    domesticShippingFee:
      localQuotation.domesticShippingFee ?? apiQuotation.domesticShippingFee,
