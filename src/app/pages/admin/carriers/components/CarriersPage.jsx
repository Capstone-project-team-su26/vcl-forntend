...
const TYPE_FILTER_OPTIONS = carrierService.CARRIER_TYPE_OPTIONS.map((option) => ({
  value: option.value,
  label: option.label,
}));
...
export default function CarriersPage() {
  ...
  async function handleDelete(item) {
    const confirmed = window.confirm(
      `Xóa đơn vị vận chuyển "${item.name}"? Hành động này không thể hoàn tác.`
    );
    if (!confirmed) return;
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        searchable: true,
        searchAccessor: (item) =>
          `${item.name || ""} ${item.code || ""} ${item.internalNotes || ""}`,
        render: (item) => (
          <div>
      ...
      },
      {
        ...
        filterAccessor: (item) => item.carrierType,
        className: "whitespace-nowrap",
        render: (item) =>
          carrierService.CARRIER_TYPE_LABELS[item.carrierType] ?? item.carrierType ?? "—",
      },
      {
        ...
        searchable: true,
        searchAccessor: (item) => formatList(item.supportedShippingMethods),
        render: (item) => (
          <span className="line-clamp-2">{formatList(item.supportedShippingMethods)}</span>
        ),
      },
      {
        key: "supportedCountriesRegions",
        title: "Quốc gia/khu vực",
        headerClassName: "hidden xl:table-cell",
        className: "text-muted hidden xl:table-cell max-w-xs",
        searchable: true,
        searchAccessor: (item) => formatList(item.supportedCountriesRegions),
        render: (item) => (
          <span className="line-clamp-2">{formatList(item.supportedCountriesRegions)}</span>
        ),
      },
      {
        key: "contactInfo",
        ...
        searchable: true,
        render: (item) => (
          <span className="line-clamp-2">{item.contactInfo || "—"}</span>
        ),
      },
      {
        key: "status",
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <p className="text-muted text-sm font-medium mt-2">
              Cấu hình carrier/forwarder để Staff và Operations chọn khi tạo lô vận chuyển quốc tế.
            </p>
          </div>
          <button
            type="button"
